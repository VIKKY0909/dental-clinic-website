...
function initializeMakeoverBeforeAfter() {
  ...
  if (
    SmileMakeover.beforeSlides.length === 0 ||
    SmileMakeover.afterSlides.length === 0
  ) {
  ...
  console.log(
    `✅ Found ${SmileMakeover.beforeSlides.length} before slides and ${SmileMakeover.afterSlides.length} after slides`
  );

  // Hide all slides initially
  SmileMakeover.beforeSlides.forEach((slide) =>
    slide.classList.remove("active")
  );
  SmileMakeover.afterSlides.forEach((slide) =>
    slide.classList.remove("active")
  );

  // Show first slide
  if (SmileMakeover.beforeSlides[0])
    SmileMakeover.beforeSlides[0].classList.add("active");
  if (SmileMakeover.afterSlides[0])
    SmileMakeover.afterSlides[0].classList.add("active");
...
function startMakeoverBeforeAfterSlideshow() {
  ...
  SmileMakeover.slideInterval = setInterval(() => {
    ...
    const nextSlideIndex =
      SmileMakeover.currentSlideIndex + 1 >= SmileMakeover.beforeSlides.length
        ? 0
        : SmileMakeover.currentSlideIndex + 1;
    ...
    setTimeout(() => {
      if (SmileMakeover.beforeSlides[SmileMakeover.currentSlideIndex]) {
        SmileMakeover.beforeSlides[
          SmileMakeover.currentSlideIndex
        ].classList.remove("active");
      }
      if (SmileMakeover.afterSlides[SmileMakeover.currentSlideIndex]) {
        SmileMakeover.afterSlides[
          SmileMakeover.currentSlideIndex
        ].classList.remove("active");
      }
      ...
      console.log(
        `Showing makeover slide pair ${SmileMakeover.currentSlideIndex + 1}/${
          SmileMakeover.beforeSlides.length
        }`
      );
...
function initializeMakeoverTestimonialsSlider() {
  ...
  SmileMakeover.testimonialsSlider = document.querySelector(
  ...
  console.log(
    "✅ Found makeover testimonials slider container, initializing..."
  );
  ...
  try {
    ...
    const testimonialsSwiper = new Swiper(".makeover-slider-dentalcontainer", {
      ...
      on: {
        init: function () {
          console.log(
            "✅ Makeover testimonials slider initialized successfully!"
          );
        },
        slideChange: function () {
          // Pause all videos when slide changes
          const allVideos = document.querySelectorAll(
            ".makeover-slider-dentalcontainer .dentaluser-video"
          );
  ...
  try {
    ...
    const peopleSwiper = new Swiper(".people-slider-wrapper", {
      ...
      on: {
        ...
        slideChange: function () {
          console.log(
            `📍 People slider: moved to slide ${this.activeIndex + 1}/${
              this.slides.length
            }`
          );
          ...
          if (
            ...
            const expectedNextIndex =
              (this.previousIndex + 1) % this.slides.length;
    ...
    console.log(
      "🎉 Makeover people slider with Swiper initialized successfully!"
    );

    // Set up periodic autoplay check to ensure it never stops and moves forward
    setInterval(() => {
      if (
        SmileMakeover.peopleSwiper &&
        SmileMakeover.peopleSwiper.autoplay &&
        !SmileMakeover.peopleSwiper.autoplay.running
      ) {
        ...
        const nextIndex =
          (currentIndex + 1) % SmileMakeover.peopleSwiper.slides.length;
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const isCorrectPage = pageIdentifiers.some(
    (selector) => document.querySelector(selector) !== null
  );

  if (!isCorrectPage) {
    console.log(
      "⚠️ Makeover page elements not found - skipping initialization"
    );
    return;
  }
...
window.addEventListener("load", function () {
  ...
  if (
    !SmileMakeover.peopleSliderInitialized ||
    !SmileMakeover.beforeAfterInitialized
  ) {
    setTimeout(() => {
      initializeAllMakeoverSliders();
    }, 100);
